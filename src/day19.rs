...
pub fn run() {
    ...
    let st1 = IfStatement::new(
        CompareGT::new('m', 2525),
        GotoStatement::new(String::from("abc")),
        Box::new(GotoStatement::new(String::from("R"))),
    );

    let p1 = Part::new(1, 2, 3, 4);
    let p2 = Part::new(1, 2526, 3, 4);

    println!("p1: {}", st1.exec(&p1));
    println!("p2: {}", st1.exec(&p2));

    let program = read_program(&lines);
    let parts = read_part_info(&lines);

    for part in parts.iter() {
        println!("Result: {}", program.check_part(part));
    }
    let total1: i32 = parts
        .iter()
        .filter(|x| program.check_part(x))
        .map(|x| x.total())
        .sum();
...
fn read_program(lines: &[String]) -> Program {
    let all_statements: HashMap<String, IfStatement> = lines
        .iter()
        .take_while(|x| !x.is_empty())
        .map(|x| {
            let (name, other) = x.split_once('{').unwrap();
            let (statement, _) = other.split_once('}').unwrap();
            (name.to_owned(), parse_statement(statement))
        })
        .collect();
    println!("map: {:#?}", all_statements);
    Program::new(all_statements)
}

fn parse_statement<'a, 'b>(txt: &'a str) -> IfStatement<'b> {
    match txt.split_once(',') {
        Some(pair) => {
            let (if_part, rest) = pair;
            let (bool_expr, then) = if_part.split_once(':').unwrap();
            if bool_expr.contains('>') {
                let (fname, amount) = bool_expr.split_once('>').unwrap();
                let field = fname.chars().next().unwrap();
                IfStatement::new(
                    CompareGT::new(field, amount.parse().unwrap()),
                    GotoStatement::new(then.to_owned()),
                    parse_else_statement(rest),
                )
            } else {
                let (fname, amount) = bool_expr.split_once('<').unwrap();
                let field = fname.chars().next().unwrap();
                IfStatement::new(
                    CompareLT::new(field, amount.parse().unwrap()),
                    GotoStatement::new(then.to_owned()),
                    parse_else_statement(rest),
                )
            }
        }
        None => panic!("Must start with boolean expression: {}", { txt }),
    }
}

fn parse_else_statement(txt: &str) -> Box<dyn Statement> {
    match txt.split_once(',') {
        Some(_) => Box::new(parse_statement(txt)),
        None => Box::new(GotoStatement::new(txt.to_owned())),
    }
}

fn read_part_info(lines: &[String]) -> Vec<Part> {
    let parts: Vec<_> = lines
        .iter()
        .skip_while(|x| !x.is_empty())
        .filter(|x| !x.is_empty())
        .map(|x| {
            let (_, data) = x.split_once('{').unwrap();
            let (data, _) = data.split_once('}').unwrap();
            Part::new(
                parse_part_value(data, 'x'),
                parse_part_value(data, 'm'),
                parse_part_value(data, 'a'),
                parse_part_value(data, 's'),
            )
        })
        .collect();
    // println!("parse: {:#?}", parts);
...
fn parse_part_value(txt: &str, val_name: char) -> i32 {
    txt.split(',')
        .filter(|c| c.starts_with(val_name))
        .map(|c| c.split_once('=').unwrap().1.parse::<i32>().unwrap())
        .next()
        .unwrap()
}
...
#[derive(Debug)]
struct Program<'a> {
    statements: HashMap<String, IfStatement<'a>>,
}

impl<'a> Program<'a> {
    fn new(statements: HashMap<String, IfStatement<'a>>) -> Self {
        Program { statements }
    }

    fn check_part(&self, part: &Part) -> bool {
        let mut next_statement = String::from("in");
        loop {
            next_statement = self.statements[&next_statement].exec(part);
            if next_statement == "A" {
                return true;
            } else if next_statement == "R" {
                return false;
            }
        }
    }
...
struct IfStatement<'a> {
    arg: Box<dyn Expression + 'a>,
    dest_true: Box<dyn Statement + 'a>,
    dest_false: Box<dyn Statement + 'a>,
}

impl Statement for IfStatement<'_> {
    fn exec(&self, part: &Part) -> String {
        if self.arg.eval(part) {
            self.dest_true.exec(part)
        } else {
            self.dest_false.exec(part)
        }
    }
}

impl<'a> IfStatement<'a> {
    fn new<T, U>(arg: T, dest_true: U, dest_false: Box<dyn Statement>) -> Self
    where
        T: Expression + 'a,
        U: Statement + 'a,
    {
...
macro_rules! comparison {
    ($name:ident, $operator:tt) => {
        ...
        impl Expression for $name {
            fn eval(&self, part: &Part) -> bool {
                if self.field == 'x' {
                    part.x $operator self.val
                } else if self.field == 'm' {
                    part.m $operator self.val
                } else if self.field == 'a' {
                    part.a $operator self.val
                } else if self.field == 's' {
                    part.s $operator self.val
                } else {
                    panic!("expected one of (x, m, a, s): {}", self.field);
                }
            }
        }

        impl $name {
            fn new(field: char, val: i32) -> Self {
                $name { field, val }
            }
        }
    };
