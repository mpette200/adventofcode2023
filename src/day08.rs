...
fn parse_network(lines: &[String]) -> HashMap<String, Info> {
    lines.iter().map(|x| parse_info(x)).collect()
}

fn parse_info(txt: &str) -> (String, Info) {
    let (p1, rest) = txt.split_once("=").unwrap();
    let (p2, p3) = rest.split_once(",").unwrap();
    (
        filter_letters(p1),
        Info {
            left: filter_letters(p2),
            right: filter_letters(p3),
        },
    )
}
