pub fn run() {
    ...
    let limit_val = Color(12, 13, 14);

    let games: Vec<Game> = lines.iter().map(|x| parse_line(x)).collect();
    ...
    let valid_games: Vec<&Game> = games
        .iter()
        .filter(|x| is_limit_inclusive(x, limit_val))
        .collect();
...
/// Includes only games where red, green and blue
/// are within the limit
fn is_limit_inclusive(game: &Game, limit: Color) -> bool {
    game.max_red <= limit.0 && game.max_green <= limit.1 && game.max_blue <= limit.2
}

/// Example:
///
/// Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green
fn parse_line(txt: &str) -> Game {
    // println!("{}", txt);
    let mut draws: Vec<Color> = Vec::new();

    let mut id_split = txt.split(":");
    let id_txt = id_split.next().unwrap();
    let id = parse_id(id_txt);

    let remaining = id_split.next().unwrap();
    for colors in remaining.split(";") {
        draws.push(parse_color(colors));
    }
    // println!("{:#?} - {:#?}", id, draws);
    // println!();
    ...
    Game {
        id,
        max_red: draws.iter().map(|x| x.0).max().unwrap(),
        max_green: draws.iter().map(|x| x.1).max().unwrap(),
        max_blue: draws.iter().map(|x| x.2).max().unwrap(),
        draws,
    }
}
...
///
/// Game 1
fn parse_id(txt: &str) -> i32 {
    let id_val = txt.rsplit(" ").next().unwrap();
    id_val.parse().unwrap()
}
...
fn parse_color(txt: &str) -> Color {
    let mut red = 0;
    let mut green = 0;
    let mut blue = 0;
    for info in txt.split(",") {
        // println!("{:#?}", parse_color_name(info));
        match parse_color_name(info) {
            ColorVal::Red(x) => red = x,
            ColorVal::Green(x) => green = x,
            ColorVal::Blue(x) => blue = x,
        }
    }
...
///
/// 3 green
fn parse_color_name(txt: &str) -> ColorVal {
    let mut splits = txt.trim().split(" ");
    let qty: i32 = splits.next().unwrap().parse().unwrap();
    match splits.next().unwrap() {
        "red" => ColorVal::Red(qty),
        "green" => ColorVal::Green(qty),
        "blue" => ColorVal::Blue(qty),
        x => panic!("bad color name: \"{}\" in string: \"{}\"", x, txt.trim()),
    }
}
